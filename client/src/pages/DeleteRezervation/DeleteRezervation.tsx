...
function DeleteRezervation() {
  ...
  const deleteRezervation = async () => {
    await fetch("http://127.0.0.1:8000/api/booking/" + params.id + "/delete", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Token " + localStorage.getItem("key"),
      ...
      {booking != null && booking != undefined ? (
        <div>
          <h3 className="text-center font-semibold">
            ...
            rezervasyonunu silmek istediğnize emin misiniz?
          </h3>
          <div className="mt-16 flex items-center justify-evenly gap-5">
            <Link to={"/rezervation/all"} className="btn">
              Geri Dön
            </Link>
...
export default DeleteRezervation;
